...
def save_recommendation_results(ranked_stocks):
...
def save_today_top_message(ranked_stocks, analysis_date):
    if len(ranked_stocks) == 0 and os.path.exists("data/today_top_recommendation.txt"):
        print("本次沒有符合門檻的股票，保留上一份今日推薦摘要，不覆蓋 today_top_recommendation.txt")
        return
...
def main():
    ...
    save_recommendation_results(ranked_stocks)
    save_today_top_message(ranked_stocks, analysis_date)
    ...
    if len(ranked_stocks) == 0:
        ...
        print("已保留上一份推薦結果，不覆蓋 Dashboard / LINE 的既有資料")
        return
